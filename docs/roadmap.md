...
gap, not a storage gap. Support DPS (Augmentation Evoker) is a third case:
its output is what *other* players do while buffed, and today the
`*_SUPPORT` events that say so are dropped as duplicates (R1).
...
and attacker, mitigation (block / dodge / parry / partial blocks / absorbs
consumed / stagger), active-mitigation uptime, self-healing, and who the
boss was hitting (the nearest thing to threat the log can express).
Augmentation wants a contribution number and per-target buff uptime, and
the players it buffs want a rank that is not inflated by it. Today every
ranking and grade the coach produces is a DPS-role number.

**What the spec fixes.** A role dimension stamped on the card (`Role` plus
a `support` flag; Augmentation stays a DPS-bucket spec ranked by
contribution); four new CONTRACT rulings — R17 damage taken and mitigation
(miss events and partial blocks parsed, a seventh `View::Taken`, by-ability
and by-attacker rows for every fight), R18 aura spans with caster and
target from a generated role-spell table (active mitigation, defensives,
externals, support buffs, major cooldowns — capped spans plus uncapped
uptime rollups), R19 support attribution (`contribution` and `net` beside
R1's untouched `damage`), R20 the shield ledger (applied vs consumed vs
wasted); a healing split and every role measure on the card; a 10 s coarse
timeline on the rows tier; role-relative `me` / `peer` grades, a `Trend {
measure }`, a `RoleNight` question; DuckDB views `taken`, `mitigation`,
`taken_spells`, `taken_sources`, `support`, `uptime`, `shields`, `coarse`
and a `role_ranks` window view that is the SQL twin of the daemon's grade;
a new fixture `support.txt` whose goldens include the conservation
identities (taken = dealt, support given = received).

**Order** (each step a PR, fixture parity green): (1) role on the card +
