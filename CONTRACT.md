...
/// R18: one (spell, caster) cell of `Segment::uptime`.
pub struct UptimeRow { pub spell_id: u32, pub label: String, pub kind: MarkKind,
...
pub enum MarkKind { TrinketUse, TrinketProc, Consumable, External,   // R12 (codes 0–3)
                    ActiveMitigation, Defensive, SupportBuff, Cooldown } // R18 (codes 4–7)
...
## Wire protocol (owner: proto) — `PROTO_VERSION = 24`
...
| 24 | R18 step 4a-ii (the socket is renamed `wowdps-v24.sock`): `Mark` + trailing string `src` — the caster's guid, empty for item marks, so a mark is now i64 at_ms | u8 kind | string label | u32 spell_id | i64 dur_ms | string src (the pinned compare frame grew by the four `00` bytes of one empty string, 0x0102 → 0x0106); `MarkKind` + `ActiveMitigation` (4), `Defensive` (5), `SupportBuff` (6), `Cooldown` (7) — codes ≥ 8 are `BadTag`; `Breakdown.timeline` on a `View::Taken` drill is `Segment::taken_timeline` (damage TAKEN on the grid, item marks + role spans; it was `None` in v21–v23), so a live Taken drill now differs from a stored one by that field until the coarse series lands on the rows tier (step 4b). Store consequence (record, not wire): every mark in `details/<id>.json` timelines writes `"src"` (empty for item marks — one stable column shape, like `misses`); a pre-v24 file without the key reads an empty caster, a mark with an unknown kind code is dropped, never re-kinded. |
