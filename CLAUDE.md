...
- `class_spells.rs` — GENERATED spell-id → class/spec table (regenerate with `python3 tools/gen-class-spells.py`, sources wago.tools DB2 exports; `tools/extract` — the `wowdps-extract` workspace crate, stdlib-only — reproduces those exports locally: WDC5 `.db2` + WoWDBDefs `.dbd` → CSV, parity-gated by `tools/extract/verify.sh`). Backs ruling R8: out of instances COMBATANT_INFO never fires, so the meter infers a player's class/spec from their casts — segment-local only (never carried forward), COMBATANT_INFO overwrites it, and it must never open a segment, or lazy/full parity breaks.
