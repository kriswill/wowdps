...
or, failing that, by the first thing only the living do (a cast, damage
dealt, healing done); one still open at the end reads to the fight's close.
It rides every timeline as a `MarkKind::Death` mark (`Segment::death_spans`,
its own map so R18's `spans`/`uptime`/`am_uptime` never see it), labelled
...
as `ClientState::compare_view`) — on Taken the tables list the abilities that
